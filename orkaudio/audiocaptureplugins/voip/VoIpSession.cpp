...
#include "../mirror_tools/libmirrortools.h"
...
void VoIpSession::Stop()
{
	...
	// Mirror tools stop session
	if (!m_callId.IsEmpty())
	{
		mirror_voip_stop((char *)m_callId.c_str(), (char *)m_trackingId.c_str());
	}
...
void VoIpSession::Start()
{
	...
	/*
	 session start，先判断m_callId是否存在，
	 存在: 表示是能正常对应到invite创建的session，可以直接使用
	 不存在: 表示session是基于rtp包创建的，需要通过ssrc来对应到具体某个callId
	 */
	if (m_callId.IsEmpty())
	{
		// 查询pkts1或者pkts2的ssrc是否已存在
		if (m_lastRtpPacketSide1)
		{
			char *id = mirror_voip_ssrc_to_call_id(m_lastRtpPacketSide1->m_ssrc);
			if (id)
			{
				if (strlen(id) > 0)
					m_callId = CStdString(id);
				mirror_voip_free_call_id(id);
			}
		}
		if (m_callId.IsEmpty() && m_lastRtpPacketSide2)
		{
			char *id = mirror_voip_ssrc_to_call_id(m_lastRtpPacketSide2->m_ssrc);
			if (id)
			{
				if (strlen(id) > 0)
					m_callId = CStdString(id);
	...
	if (!m_callId.IsEmpty())
	{
		mirror_voip_add_tracking((char *)m_callId.c_str(), (char *)m_capturePort.c_str());
		// 添加pkts1和pkts2的ssrc
		if (m_lastRtpPacketSide1)
			mirror_voip_add_ssrc((char *)m_callId.c_str(), m_lastRtpPacketSide1->m_ssrc);
		if (m_lastRtpPacketSide2)
			mirror_voip_add_ssrc((char *)m_callId.c_str(), m_lastRtpPacketSide2->m_ssrc);
		...
		//  	{
		//  		if (strlen(id) > 0) {
		//  			LOG4CXX_INFO(m_log, "Test CallID len: [" + IntToString(strlen(id)) + "]");
		//  			m_callId = CStdString(id);
		//  		}
		//  		mirror_voip_free_call_id(id);

		// 		LOG4CXX_INFO(m_log, "Test CallID: [" + m_callId + "]");
		// 	}
		// }
	}
...
bool VoIpSession::AddRtpPacket(RtpPacketInfoRef &rtpPacket)
{
	...
	if (m_started)
	{
		...
		// Mirror tools add rtp packet
		VoIpRTPData data;
		data.call_id = (char *)m_callId.c_str();
		data.channel = channel;
		data.source_ip = rtpPacket->m_sourceIp.s_addr;
		data.dest_ip = rtpPacket->m_destIp.s_addr;
		data.source_port = rtpPacket->m_sourcePort;
		data.dest_port = rtpPacket->m_destPort;
		data.payload_type = rtpPacket->m_payloadType;
		data.payload = rtpPacket->m_payload;
		data.payload_size = rtpPacket->m_payloadSize;
		data.ssrc = rtpPacket->m_ssrc;
		mirror_voip_rtp(data);
...
void VoIpSession::ReportSipInvite(SipInviteInfoRef &invite)
{
	if (DLLCONFIG.m_sipMetadataUseLastInvite || m_invite.get() == NULL)
	{
		...
		CStdString logMsg;
		invite->ToString(inviteString);
		logMsg.Format("[%s] ReportSipInvite INVITE:%s", m_trackingId, inviteString);
		LOG4CXX_INFO(m_log, logMsg);
	}
	else
	{
...
{
	// 2023-07-04 北京银行外呼系统的SIP请求流程
	// 1.媒体服务器：172.17.7.82 坐席分机：160066 注册到媒体服务器
	// 2.外呼时媒体服务器from:6666618612319397@172.17.7.82 向坐席分机发起INVITE，坐席分机 to: 160066@10.128.40.63，此时INVITE中的RTP信息为rtpIp:172.17.7.82 rtpPort:10062
	// 媒体服务器外呼都会先通过10062端口，然后再重新reINVITE到其他媒体服务器上
	// 3.第二次INVITE，媒体服务器from: CallingSub@172.17.7.82 请求坐席话机，to:LineInfo@10.128.40.63
	// 4.第三次INVITE，真正分配rtp端口及媒体服务器的请求(或者说接通外呼线路)，媒体服务器from:6666618612319397@172.17.7.82 向坐席分机发起INVITE to:160066@10.128.40.63 此时INVITE中的RTP信息为rtpIp:172.17.7.93 rtpPort:50508
	// 5.坐席话机的rtpPort基本都是30000
	...
	// 2023-07-04 北京银行过滤rtp端口
	// 处理第一次及第二次的INVITE请求，只保留第三次的INVITE请求
	if (rtpPort == DLLCONFIG.m_ignoredInviteRTPPort || rtpPort == 0)
	{
		LOG4CXX_INFO(m_log, "INVITE ignored by rtpPort 0 or " + IntToString(DLLCONFIG.m_ignoredInviteRTPPort));
		return;
	}
	...
	if (session.get() == NULL && DLLCONFIG.m_sipTrackMediaAddressOnSender)
	{
		...
		session = findByMediaAddress(invite->m_originalSenderIp, rtpPortAsInt);
	}
	if (session.get())
	{
	...
	// Mirror tools start session
	VoIpSipData data;
	data.call_id = (char *)invite->m_callId.c_str();
	CStdString localParty = VoIpSessionsSingleton::instance()->GetLocalPartyMap(invite->m_from);
	CStdString remoteParty = VoIpSessionsSingleton::instance()->GetLocalPartyMap(invite->m_to);
	data.local_party = (char *)localParty.c_str();
	data.remote_party = (char *)remoteParty.c_str();
	if (!invite->m_fromName.IsEmpty())
	{
		data.from_name = (char *)invite->m_fromName.c_str();
	}
	if (!invite->m_requestUri.IsEmpty())
	{
		data.request_uri = (char *)invite->m_requestUri.c_str();
	}

	data.rtp_ip = invite->m_fromRtpIp.s_addr;
	data.rtp_port = std::atoi(invite->m_fromRtpPort);
	data.sender_ip = invite->m_senderIp.s_addr;
	data.receiver_ip = invite->m_receiverIp.s_addr;

	mirror_voip_invite(data, (char *)trackingId.c_str());
}
...
void VoIpSessions::ReportSip200Ok(Sip200OkInfoRef info)
{
	...
	LOG4CXX_INFO(m_log, "ReportSip200Ok: " + logMsg);

	// for (auto iter = m_byCallId.begin(); iter != m_byCallId.end(); iter++)
	// {
	// 	LOG4CXX_INFO(m_log, "CurrentCallId: " + iter->second->m_callId);
	// }
