...
void CapturePort::AddCaptureEvent(CaptureEventRef eventRef)
{
	...
	if (!audioTapeRef.get())
	{
		if (!CONFIG.m_vad && !CONFIG.m_audioSegmentation)
		{
			...
			LOG4CXX_WARN(s_log, "#" + m_id + ": received unexpected capture event:" + CaptureEvent::EventTypeToString(eventRef->m_type));
		}
	}
	else
	{
		...
		case CaptureEvent::EtStop:
		{
			...
			if (CONFIG.m_holdResumeReportDuration)
				audioTapeRef->PopulateTag("holdduration", IntToString(audioTapeRef->m_holdDuration));
